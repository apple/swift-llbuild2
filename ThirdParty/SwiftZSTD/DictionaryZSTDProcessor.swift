...
public class DictionaryZSTDProcessor
{
    ...
    var compDict : OpaquePointer? {
        ...
        if Junk.retVal == nil {
            try! dict.withUnsafeBytes { (p : UnsafeRawBufferPointer) throws in
                Junk.retVal = ZSTD_createCDict(p.baseAddress, p.count, compLevel)
                    haveCDict = true
            }
        }
        return Junk.retVal
    ...
    var decompDict : OpaquePointer? {
        ...
        if Junk.retVal == nil {
            try! dict.withUnsafeBytes { (p : UnsafeRawBufferPointer) throws in
                Junk.retVal = ZSTD_createDDict(p.baseAddress, p.count)
                haveDDict = false
            }
        }
        return Junk.retVal
     ...
     */
    public init?(withDictionary: Data, andCompressionLevel: Int)
    {
        ...
        dict = withDictionary
        compLevel = CInt(andCompressionLevel)
    }
