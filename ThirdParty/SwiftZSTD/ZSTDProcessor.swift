...
public class ZSTDProcessor
{
     ...
     */
    public func compressBuffer(_ dataIn : Data, compressionLevel : Int) throws -> Data
    {
        guard isValidCompressionLevel(compressionLevel) else {
            throw ZSTDError.invalidCompressionLevel(level: compressionLevel)
        }
        currentCL = CInt(compressionLevel)
