// This source file is part of the Swift.org open source project
//
// Copyright (c) 2020 Apple Inc. and the Swift project authors
// Licensed under Apache License v2.0 with Runtime Library Exception
//
// See http://swift.org/LICENSE.txt for license information
// See http://swift.org/CONTRIBUTORS.txt for the list of Swift project authors

import Dispatch
import TSCUtility
import FXCore
import FXAsyncSupport
import XCTest

class InMemoryCASDatabaseTests: XCTestCase {
    let group = FXMakeDefaultDispatchGroup()

    func testBasics() throws {
        let db = FXInMemoryCASDatabase(group: group)
        let ctx = Context()

        let id1 = try db.put(data: FXByteBuffer.withBytes([1, 2, 3]), ctx).wait()
        let obj1 = try db.get(id1, ctx).wait()!
        XCTAssertEqual(id1, FXDataID(string: "0~sXfsG_Jt-ztwENRz5tRHE7KbdluZxuYOy_rnQt5JZUM="))
        XCTAssertEqual(obj1.size, 3)
        XCTAssertEqual(obj1.refs, [])
        XCTAssertEqual(obj1.data, FXByteBuffer.withBytes([1, 2, 3]))

        let id2 = try db.put(refs: [id1], data: FXByteBuffer.withBytes([4, 5, 6]), ctx).wait()
        let obj2 = try db.get(id2, ctx).wait()!
        XCTAssertEqual(id2, FXDataID(string: "0~udZrZzFHJr8uovWT5dOWtKz95ZqKi-vBkpiH0mJfjM4="))
        XCTAssertEqual(obj2.size, 3)
        XCTAssertEqual(obj2.refs, [id1])
        XCTAssertEqual(Array(buffer: obj2.data), [4, 5, 6])
        XCTAssertEqual(try db.contains(id1, ctx).wait(), true)

        // Check contains on a missing object.
        let missingID = try FXInMemoryCASDatabase(group: group).put(
            data: FXByteBuffer.withBytes([]), ctx
        ).wait()
        XCTAssertEqual(try db.contains(missingID, ctx).wait(), false)
    }

    func testPutStressTest() throws {
        let db = FXInMemoryCASDatabase(group: group)
        let ctx = Context()
        let queue = DispatchQueue(label: "sync")

        // Insert one object.
        let id1 = try db.put(data: FXByteBuffer.withBytes([1, 2, 3]), ctx).wait()

        // Insert a bunch of objects concurrently.
        //
        // We take care here to do this in a way that no references to the
        // object data is held (other than in the database).
        let allocator = FXByteBufferAllocator()
        func makeData(i: Int, objectSize: Int = 16) -> FXByteBuffer {
            var buffer = allocator.buffer(capacity: objectSize)
            for j in 0..<objectSize {
                buffer.writeInteger(UInt8((i + j) & 0xFF))
            }
            return buffer
        }
        let numObjects = 100
        var objects = [FXDataID?](repeating: nil, count: numObjects)
        DispatchQueue.concurrentPerform(iterations: numObjects) { i in
            let id = { try! db.put(refs: [id1], data: makeData(i: i), ctx).wait() }()
            queue.sync {
                objects[i] = id
            }
        }

        for i in 0..<numObjects {
            guard let result = try db.get(objects[i]!, ctx).wait() else {
                XCTFail("missing expected object")
                return
            }
            XCTAssertEqual(result.refs, [id1])
            XCTAssertEqual(result.data, makeData(i: i))
        }
    }
}
