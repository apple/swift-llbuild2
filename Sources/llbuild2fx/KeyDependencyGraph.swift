...
public class FXKeyDependencyGraph: @unchecked Sendable {
    ...
    private var knownKeys: [Int: FXRequestKey]
    ...
    public func addEdge(from origin: FXRequestKey, to destination: FXRequestKey) throws {
        ...
        if let path = anyPath(from: destinationID, to: originID) {
            ...
            lock.unlock()
            throw FXError.cycleDetected([origin] + keyPath)
        }
    ...
    public func removeEdge(from origin: FXRequestKey, to destination: FXRequestKey) {
        let originID = origin.hashValue
        let destinationID = destination.hashValue
