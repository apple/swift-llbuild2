...
package struct ProcessExitExtendedInfo: Sendable {
...
package enum ProcessExitReason: Hashable & Sendable {
    case exit(CInt)
    case signal(CInt)

    package func throwIfNonZero() throws {
        switch self {
        case .exit(0):
            return
        default:
...
extension ProcessExitReason {
    ...
    package var asShellExitCode: Int {
        switch self {
        case .exit(let code):
            return Int(code)
    ...
    package var asPythonExitCode: Int {
        switch self {
        case .exit(let code):
            return Int(code)
        case .signal(let code):
...
package struct ProcessExecutionError: Error & Hashable & Sendable {
    package var exitReason: ProcessExitReason

    package init(_ exitResult: ProcessExitReason) {
