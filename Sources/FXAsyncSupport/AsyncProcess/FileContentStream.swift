...
package struct _FileContentStream: AsyncSequence & Sendable {
    package typealias Element = ByteBuffer
    typealias Underlying = AsyncThrowingChannel<Element, Error>

    package final class AsyncIterator: AsyncIteratorProtocol {
        package typealias Element = ByteBuffer
    ...
    package struct IOError: Error {
        package var errnoValue: CInt

        package static func makeFromErrnoGlobal() -> IOError {
...
extension AsyncSequence where Element == ByteBuffer, Self: Sendable {
    ...
    package var strings: AsyncMapSequence<Self, String> {
        return self.map { String(buffer: $0) }
    }
}

package struct AsyncByteBufferLineSequence<Base: Sendable>: AsyncSequence & Sendable
