...
package struct ProcessOutputStream: Sendable & Hashable & CustomStringConvertible {
    ...
    package static let standardOutput: Self = .init(backing: .standardOutput)

    package static let standardError: Self = .init(backing: .standardError)
...
package struct ProcessOutput: Sendable {
    ...
    package static let discard: Self = .init(backing: .discard)

    /// Inherit the same file description from the parent process (i.e. this process).
    package static let inherit: Self = .init(backing: .inherit)
    ...
    package static func fileDescriptor(takingOwnershipOf fd: FileDescriptor) -> Self {
    ...
    package static func fileDescriptor(sharing fd: FileDescriptor) -> Self {
    ...
    package static let stream: Self = .init(backing: .stream)
}
...
package final actor ProcessExecutor {
    ...
    package static var isBackedByPSProcess: Bool {
        return Process.self == PSProcess.self
    }

    package struct SpawnOptions: Sendable {
        ...
        package var closeOtherFileDescriptors: Bool

        /// Change the working directory of the child process to this directory.
        package var changedWorkingDirectory: Optional<String>
        ...
        package var createNewSession: Bool
        ...
        package var ignoreStdinStreamWriteErrors: Bool
        ...
        ///
        /// Default is `true`.
        package var cancelProcessOnStandardInputWriteFailure: Bool
        ...
        package var cancelStandardInputWritingWhenProcessExits: Bool

        /// Safe & sensible default options.
        package static var `default`: SpawnOptions {
            return SpawnOptions(
    ...
    package struct OSError: Error & Sendable & Hashable {
        package var errnoNumber: CInt
    ...
    package struct TeardownSequence: Sendable, ExpressibleByArrayLiteral, CustomStringConvertible {
        package typealias ArrayLiteralElement = TeardownStep

        package init(arrayLiteral elements: TeardownStep...) {
            self.steps = (elements.map { $0.backing }) + [.kill]
        }

        package struct TeardownStep: Sendable {
            var backing: Backing
            ...
            package static func sendSignal(_ signal: CInt, allowedTimeToExitNS: UInt64) -> Self {
    ...
    package var standardOutput: ChunkSequence {
        get async {
    ...
    package var standardError: ChunkSequence {
        get async {
    ...
    package init<StandardInput: AsyncSequence & Sendable>(
    ...
    package func run() async throws -> ProcessExitReason {
    ...
    /// If you prefer to get the standard output and error in one (non-stremed) piece upon exit, consider the `static` methods such as
    /// ``ProcessExecutor/runCollectingOutput(group:executable:_:standardInput:collectStandardOutput:collectStandardError:perStreamCollectionLimitBytes:environment:logger:)``.
    package func runWithExtendedInfo() async throws -> ProcessExitExtendedInfo {
    ...
    package nonisolated var bestEffortProcessIdentifier: pid_t? {
    ...
    package func sendSignal(_ signal: CInt) async throws {
...
extension ProcessExecutor {
    ...
    package static var defaultEventLoopGroup: any EventLoopGroup {
    ...
    package static var disableLogging: Logger {
