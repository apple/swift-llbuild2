...
package struct OutputLoggingSettings: Sendable {
    /// Where should the output line put to?
    package enum WhereTo: Sendable {
    ...
    package var to: WhereTo

    package init(logLevel: Logger.Level, to: OutputLoggingSettings.WhereTo) {
...
extension ProcessExecutor {
    ...
    package static func run<StandardInput: AsyncSequence & Sendable>(
    ...
    package static func runLogOutput<StandardInput: AsyncSequence & Sendable>(
        group: EventLoopGroup = ProcessExecutor.defaultEventLoopGroup,
        executable: String,
        _ arguments: [String],
        standardInput: StandardInput,
    ...
    ///   - logger: Where to log diagnostic and output messages to
    package static func runProcessingOutput<StandardInput: AsyncSequence & Sendable>(
        group: EventLoopGroup = ProcessExecutor.defaultEventLoopGroup,
        executable: String,
        _ arguments: [String],
        spawnOptions: SpawnOptions = .default,
    ...
    package struct TooMuchProcessOutputError: Error, Sendable & Hashable {
        package var stream: ProcessOutputStream
    }

    package struct ProcessExitReasonAndOutput: Sendable & Hashable {
        ...
        package var exitReason: ProcessExitReason

        /// Any errors that occurred whilst writing the provided `standardInput` sequence into the child process' standard input.
        package var standardInputWriteError: Optional<any Error>

        package var standardOutput: ByteBuffer?
        package var standardError: ByteBuffer?
    }
    ...
    ///   - logger: Where to log diagnostic and output messages to
    package static func runCollectingOutput<StandardInput: AsyncSequence & Sendable>(
        group: EventLoopGroup = ProcessExecutor.defaultEventLoopGroup,
        executable: String,
        _ arguments: [String],
        spawnOptions: SpawnOptions = .default,
...
extension ProcessExecutor {
    ...
    ///   - logger: Where to log diagnostic and output messages to
    ///   - logConfiguration: How to log the output lines
    package static func runLogOutput(
        group: EventLoopGroup = ProcessExecutor.defaultEventLoopGroup,
        executable: String,
        _ arguments: [String],
        environment: [String: String] = [:],
    ...
    ///   - splitOutputIntoLines: Whether to call the closure with full lines (`true`) or arbitrary chunks of output (`false`)
    ///   - logger: Where to log diagnostic and output messages to
    package static func runProcessingOutput(
        group: EventLoopGroup = ProcessExecutor.defaultEventLoopGroup,
        executable: String,
        _ arguments: [String],
        spawnOptions: SpawnOptions = .default,
    ...
    ///   - collectStandardError: If `true`, collect all of the child process' standard error into memory, discard if `false`
    ///   - logger: Where to log diagnostic and output messages to
    package static func runCollectingOutput(
        group: EventLoopGroup = ProcessExecutor.defaultEventLoopGroup,
        executable: String,
        _ arguments: [String],
        spawnOptions: SpawnOptions = .default,
