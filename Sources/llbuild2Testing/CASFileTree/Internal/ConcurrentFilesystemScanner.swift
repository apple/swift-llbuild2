...
class ConcurrentFilesystemScanner: Sequence {
    package typealias Iterator = FilesystemIterator
    package typealias Element = (path: AbsolutePath, type: FilesystemObjectType)
    ...
    package func makeIterator() -> FilesystemIterator {
...
struct FilesystemPathInfo {
    ...
    package let info: PathInfo

    package enum PathInfo {
        /// A plain file.
        case file
        /// A symbolic link.
        case symlink
    ...
    package var type: FilesystemObjectType {
        switch info {
        case .file:
            return .REG
        case .symlink:
    ...
    package init(directory path: AbsolutePath) throws {
...
class FilesystemDirectoryIterator: IteratorProtocol {
    ...
    package typealias Element = ([NameAndType], hasMoreEntries: Bool)
    ...
    #endif
    package typealias NameAndType = (name: String, type: FilesystemObjectType)
    ...
    package init(_ path: AbsolutePath) throws {
...
package enum FilesystemObjectType: UInt8 {
    case UNKNOWN
    case FIFO
    case CHR
    case DIR
