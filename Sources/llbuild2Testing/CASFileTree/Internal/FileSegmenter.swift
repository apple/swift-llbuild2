...
internal final class FileSegmenter {
    ...
    init(
        ...
        var fd = try FXFutureFileSystem.openImpl(path.pathString, flags: O_RDONLY | O_NOFOLLOW)
    ...
    func fetchSegment(segmentNumber: Int) throws -> (LLBFastData, isEOF: Bool)? {
        ...
        default:
            let newFD: CInt
            do {
                newFD = try FXFutureFileSystem.openImpl(
        ...
        let data = try FXFutureFileSystem.syncReadComplete(
